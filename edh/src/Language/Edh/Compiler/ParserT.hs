...
module Language.Edh.Compiler.ParserT where
...
import qualified Language.Edh.Common.Stream    as S
...
instance Monad m => Alternative (ParserT s m) where
    empty = undefined -- fail "empty" -- not really an identity
...
next = preview << consume >>= returnOrThrow
    (ParserError "unexpected end of stream")

atom :: (Monad m, S.Stream s a, Show a, Eq a) => a -> ParserT s m a
atom a = do
    a' <- next
    if a == a' then return a else fail $ "fail to parse " ++ show a
...
string s = go s $> s
  where
    ...
    go s' = case S.read s' of
        ...
        Just (a, s'') -> do
            a' <- next
            if a == a' then go s'' else fail $ "fail to parse " ++ show s
...
predicate f = do
    a <- next
    if f a then return a else fail $ "unexpected " ++ show a

many :: Monad m => ParserT s m a -> ParserT s m [a]
many parser = do
    maybe' <- (Just <$> parser)
        `catchError` \(_ :: ParserError) -> return Nothing
    case maybe' of
...
execParserT
    :: (Monad m, S.Stream s a) => ParserT s m x -> s -> m (Either ParserError x)
execParserT parser s =
    runExceptT (runStateT (runParserT parser) (ParserState s)) >>= \case
